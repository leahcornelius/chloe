...
async def on_ready():
    ...
    print('Logged in as '+client.user.name+' (ID:'+str(client.user.id) +
          ') | '+str(len(client.guilds))+' servers | ' + getAllUsersCount())
...
async def on_message(message):
    ...
    global number_of_sent_messages
    global number_of_servers
    # Check to ensure the bot does not respond to its own messages
    if (
        ...
        and (
            ...
            or NEED_MENTION != False
            or (message.author.id == int(OWNER_ID) and OWNER_BYPASS_MENTION)
        )
    ):
        async with message.channel.typing():  # Show that the bot is typing
            ...
            dicestr = re.search(
                "Roll (\d{1,2})d(\d{1,3})", message.content)
            if (dicestr != None):
                dice = [dicestr.group(1), dicestr.group(2)]
                output = "I rolled "
                for _ in range(int(dice[0])):
                    output += str(random.randrange(1,
                                                   int(dice[1]))) + ", "
                txt = output
            else:
                ...
                lang = "en"
                if(lang != "en"):
                    txtinput = str(translator.translate(
                        txtinput, dest="en", src=lang).text)
                # _context.append(txtinput)
                if(isinstance(message.channel, discord.abc.PrivateChannel)):
                    # Get a response!
                    txt = get_response(
                        txtinput, message.author.id, False)
                else:
                    # Get a response!
                    txt = get_response(
                        txtinput, message.guild.id, False)
...
def getAllUsersCount():
    ...
    user_count = sum(len(g.members) for g in guilds)
...
def write_status_report():
    ...
    with open("./status_report.txt", "w") as f:
        ...
        f.write("Number of messages received since last reboot: " +
                str(number_of_messages) + "\n")
        f.write("Number of messages sent since last reboot: " +
                str(number_of_sent_messages) + "\n")
        f.write("Number of failed responses since last reboot: " +
                str(number_of_messages - number_of_sent_messages) + "\n")
        f.write("Number of guilds in memory: " +
                str(len(history_dict)) + "\n```")
        f.close()
...
def get_response(prompt, channel_id, do_infinite):
    ...
    if channel_id not in history_dict:
    ...
    history = "".join(message + tokenizer.eos_token for message in static_history)
